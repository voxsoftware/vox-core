...
(function() {
    ...
    var Decimal = function(v) {
        ...
        if (e_pos != -1) {
            exp = -parseInt(v_str.substring(e_pos + 1), 10);
            if (isNaN(exp)) {
        ...
        var adjustment = Decimal.decimal_digits -
        (v_str.length > exp ? v_str.length : exp);
		//vw.info(v_str.length+adjustment);
        var carry = 0;
        ...
        if (adjustment > 0) {
            ...
            var last_digit_str = v_str.substring(Decimal.decimal_digits - 1,
                                                 Decimal.decimal_digits);
            var extra_digit_str = v_str.substring(Decimal.decimal_digits,
                                                 Decimal.decimal_digits + 1);
            var last_digit = parseInt(last_digit_str);
            var extra_digit = parseInt(extra_digit_str);
            if (!isNaN(last_digit) && last_digit % 2 == 1 &&
                !isNaN(extra_digit) && extra_digit >= 5) {
                carry = 1;
            }
            v_str = v_str.substring(0, Decimal.decimal_digits);
        }

        exp += adjustment;
		//vw.info("..",exp," - ",v_str);

		var relleno= v_str.length-Decimal.maxdecimal_digits;
		v_str=v_str.substring(0,Decimal.maxdecimal_digits);
		for(var i=0;i<relleno;i++){
			v_str+='0';
		}


        if (v_str.length - exp > Decimal.maxdecimal_digits) {
        ...
        for (var i = Decimal.n - 1; i >= 0; --i) {
            var num = parseFloat(v_str.substring(i *  Decimal.decimal_digits_per_word, (i + 1) * Decimal.decimal_digits_per_word)) + carry;
            if (num == Decimal.one_word) {
                num = 0;
            }
            else {
                carry = 0;
    ...
    Decimal.decimal_digits_per_word = 9;
    Decimal.n = 4;
    Decimal.decimal_digits = Decimal.decimal_digits_per_word * Decimal.n;
	Decimal.maxdecimal_digits=29;
	Decimal.minimumexp=7;
	Decimal.decimal_restrict=2; // Esta propiedad indica cual es la cantidad de digitos restringidos para el array sig posición 0
    ...
    Decimal.ArgumentError = function() {
    ...
    Decimal.fromData = function(sig, exp, is_minus) {
        ...
        //if (obj.exp < -1) {
		if (obj.exp < Decimal.minimumexp) { // Se coloca esto para permitir solo 29 dígitos
            throw new Decimal.OverflowError();
        }
		...
		// Esto es para no permitir una precisión mayor a 29 dígitos

		var val= obj.sig[0].toString();
		val=val.substring(0,Decimal.decimal_restrict);
		if(val.length>=Decimal.decimal_restrict){
			for(var i=Decimal.decimal_restrict;i<Decimal.decimal_digits_per_word;i++){
				val+='0'
			}
			obj.sig[0]=val|0;
		}
    ...
    Decimal.Abs = function(obj) {
        var r = new Decimal(obj);
        r.is_minus = false;
        return r;
    };

    Decimal.Floor = function(obj) {
        return Decimal.RoundInternal(obj, 0, obj.is_minus ?
                                     Decimal.MidpointRounding.Ceil :
                                     Decimal.MidpointRounding.Floor);
    };

    Decimal.Ceiling= Decimal.Ceil = function(obj) {
        return Decimal.RoundInternal(obj, 0, obj.is_minus ?
                                     Decimal.MidpointRounding.Floor :
                                     Decimal.MidpointRounding.Ceil);
    };

    Decimal.Truncate = function(obj) {
        return Decimal.RoundInternal(obj, 0, Decimal.MidpointRounding.Floor);
    };
    ...
    Decimal.round = Decimal.Round;  // deprecated; for backword compatibility only
    ...
    Decimal.RoundInternal = function(obj, digits, mode) {
        ...
        if (mode == Decimal.MidpointRounding.Floor) {
            ...
            var last_digit_is_even = (round_word_pos == Decimal.n) // 0.5
            || (Math.floor(obj.sig[round_word_pos] / tmp) % 2 == 0);
        ...
        if (round_up) {
            if (round_word_pos == Decimal.n) {  // only when 0.5
                ret.sig[round_word_pos - 1] = Decimal.one_word / 10;
                --ret.exp;
            } else {
    ...
    Decimal.prototype.compareTo=Decimal.prototype.compare = function(another) {
    ...
    Decimal.prototype.div = function(another) {
        ...
        if (another_decimal.isZero()) {
            throw new Decimal.ZeroDivisionError();
        }
    ...
    Decimal.prototype.mod = function(other) {
        var other_decimal = (other instanceof Decimal ? other :
                             new Decimal(other));
        var a = Decimal.Abs(this);
        var b = Decimal.Abs(other_decimal);
        var r = a.sub(Decimal.Truncate(a.div(b)).mul(b));
